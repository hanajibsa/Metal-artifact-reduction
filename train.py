...
def train_syndiff(rank, gpu, args):
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for iteration, (x1, x2) in enumerate(data_loader):
            ...
            errG = 2 * errG_adv + errG_cycle_adv + errG_cycle + 2 * errG_L1 + errG_recon + errG_art
            errG.backward()
            ...
            if iteration % 100 == 0:
                if rank == 0:
                    print('epoch {} iteration{}, G-Adv: {}, G-cycle-Adv: {}, G-Cycle: {}, G-L1: {}, G-Sum: {}, D Loss: {}, D_cycle Loss: {}'.format(epoch, iteration, errG_adv.item(), errG_cycle_adv.item(), errG_cycle.item(), errG_L1.item(), errG.item(), errD.item(), errD_cycle.item()))
