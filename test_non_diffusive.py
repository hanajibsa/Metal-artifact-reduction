...
def sample_from_model(coefficients, generator, n_time, x_init, T, opt):
    ...
    with torch.no_grad():
        for i in reversed(range(n_time)):
            ...
            x_0 = generator(x_init, t_time, latent_z)
...
def sample_and_test(args):
    ...
    #loading dataset
    phase='test'
    dataset=CreateDatasetSynthesis(args.add_path, args.input_path, args.contrast1, args.contrast2)
    data_loader = torch.utils.data.DataLoader(dataset,
                                               batch_size=1,
                                               shuffle=False,
                                               num_workers=16)
    #Initializing and loading network
    gen_non_diffusive = adn.define_G(gpu_ids=[0])
    ...
    # load_checkpoint(checkpoint_file, gen_non_diffusive_1to2,'gen_non_diffusive_1to2',epoch=str(epoch_chosen), device = device)
    load_checkpoint(checkpoint_file, gen_non_diffusive,'gen_non_diffusive',epoch=str(epoch_chosen), device = device)
    ...
    save_dir = exp_path + "/generated_samples/non_diffusive/epoch_{}".format(epoch_chosen)
    ...
    for iteration, (x , y) in enumerate(data_loader): 
        ...
        # print('real_data:', real_data.shape, 'source_data: ', source_data.shape)
        
        fake_sample2, _, _, _ = gen_non_diffusive(source_data, real_data)
        ...
        torchvision.utils.save_image(fake_sample2, '{}/sample2_{}.jpg'.format(save_dir, files_name[iteration]), normalize=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--add_path', type=str, default='test'),
    parser.add_argument('--file_name_path')
